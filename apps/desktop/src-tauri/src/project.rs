...
pub fn new_project(project: Project) -> Result<(), String> {
	...
	std::fs::create_dir_all(path.join("backups")).map_err(|_| "Error creating backups directory".to_owned())?;
	std::fs::write(
		path.join(format!("{}.wlfr", project.database.root().name)),
...
pub fn save_project(project: Project) -> Result<(), String> {
	std::fs::write(
		std::path::Path::new(&project.location).join(format!("{}.wlfr", project.database.root().name)),
		project.to_bytes().map_err(|_| "Error serializing project".to_owned())?,
	)
