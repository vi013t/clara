...
export interface Shape<T extends Shape<T>> extends Cloneable<T> {
	overlaps(other: Shape<any>, padding?: number): boolean;
	contains(other: Shape<any>, padding?: number): boolean;
}
...
export class Circle implements Shape<Circle> {
	...
	public overlaps(other: Shape<any>, padding: number = 0): boolean {
	...
	public contains(other: Shape<any>, padding: number = 0): boolean {
	...
	public static prettyPack(smallCircles: number, smallCircleRadius: number): PackingResult {
		// Base Cases
